'''
CNN model for the flowers dataset (17 classes - 1360 image)
it doesnt use the point of interest method
it works perfectly
'''
...
def load_image(file_path):
    # resize to 224x224 for training purposes
...
images = []
labels = np.zeros((1360, 17))
j = -1
# creating images and labels
for i in range(0, 1360):
    fname="./flower dataset/dataset/image_"
    fpath = os.path.join(fname + str(i+1).zfill(4) + ".jpg")
...
weights = {
    ...
    'W3': tf.get_variable('W3', shape=(32*32*16, 1024), initializer=tf.contrib.layers.xavier_initializer()),
    'W4': tf.get_variable('W4', shape=(1024, 256), initializer=tf.contrib.layers.xavier_initializer()),
...
biases = {
    ...
    'B3': tf.get_variable('B3', shape=(1024), initializer=tf.contrib.layers.xavier_initializer()),
    'B4': tf.get_variable('B4', shape=(256), initializer=tf.contrib.layers.xavier_initializer()),
    'B5': tf.get_variable('B5', shape=(17), initializer=tf.contrib.layers.xavier_initializer()),
}
...
def conv_net(x, weights, biases):
    ...
    fc1 = tf.reshape(conv4, [-1, 32*32*16])
